...
variable "vpc_cidr_block" {
  ...
  type        = string
  default     = "10.100.0.0/16" # Different CIDR for prod to avoid overlap if peering
}
...
variable "rds_instance_class" {
  ...
  type        = string
  default     = "db.t3.micro" # Keep micro for cost, can be upgraded
}
...
variable "rds_skip_final_snapshot" {
  ...
  type        = bool
  default     = false # Do NOT skip final snapshot for production
}
...
variable "lambda_log_retention_days" {
  ...
  type        = number
  default     = 14 # Longer retention for production
}
